...
Procedure Show_Help;
Begin
  WriteLn ('Usage: GBBSUTIL <Options>');
  WriteLn;
...
Begin
  ...
  Assign (ConfigFile, 'vertigo.dat');

  {$I-} Reset(ConfigFile); {$I+}

  If IoResult <> 0 Then Begin
    WriteLn ('Must be executed in same directory as VERTIGO.DAT file');
    Halt(1);
  End;
  ...
  If Config.DataChanged <> mysDataChanged Then Begin
    WriteLn ('This program is not compatible with the current version of Vertigo');
    Halt(1);
  End;
...
Begin
  ...
  WriteLn;
  WriteLn ('GBBSUTIL: ', mysSoftwareID, ' Utilities Version ', mysVersion, ' (', OSID, ')');
  WriteLn;
  ...
  If IoResult <> 0 Then Begin
    WriteLn ('Error reading VERTIGO.DAT.  Run GBBSUTIL from the main BBS directory.');
    Halt(1);
  End;
    ...
    For A := 1 to Config.INetTNNodes Do Begin
      ...
      If IoResult = 0 Then Begin
        ...
        If Chat.Active Then Begin
          WriteLn ('ERROR: GBBSUTIL has detected that a user is online at this time.');
          ...
          WriteLn;
          WriteLn ('If there are NO users online and GBBSUTIL detects that there are, try');
