...
Begin
  ...
  Assign (Session.ConfigFile, bbsCfgPath + 'vertigo.dat');
End;
...
Var
  Info : Stat;
Begin
  If fpStat('vertigo', Info) = 0 Then Begin
...
*)
  ...
  If IoResult <> 0 Then Begin
    Console.WriteLine ('ERROR: No message base configuration. Use VERTIGO -CFG');
    DisposeClasses;
    Halt(1);
  End;
...
Begin
  {$IFDEF DEBUG}
    SetHeapTraceOutput('vertigo.mem');
  {$ENDIF}
  ...
  If Session.ConfigMode Then Begin
    ...
    Console.SetWindowTitle ('Vertigo Configuration');
