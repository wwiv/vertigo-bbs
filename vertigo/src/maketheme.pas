...
Begin
  ...
  If IoResult <> 0 Then Begin
    WriteLn;
    WriteLn;
    WriteLn ('ERROR: Cannot run while Vertigo is loaded');
    Halt(1);
  End;
...
Begin
  WriteLn;
  WriteLn ('MAKETHEME : Vertigo BBS Theme Compiler Version ' + mysVersion);
  WriteLn;
  ...
  If ParamCount < 2 Then Begin
    ...
    WriteLn ('   COMPILE : Compiles [Input File] into a Vertigo Theme file');
  ...
  If IoResult <> 0 Then Begin
    BasePath := GetENV('vertigobbs');

    If BasePath <> '' Then BasePath := DirSlash(BasePath);

    Assign (ConfigFile, BasePath + 'vertigo.dat');
    Reset  (ConfigFile);

    If IoResult <> 0 Then Begin
      WriteLn ('ERROR: Unable to read VERTIGO.DAT');
      WriteLn;
      WriteLn ('VERTIGO.DAT must exist in the same directory as MakeTheme, or in the');
      WriteLn ('path defined by the VERTIGOBBS environment variable.');
      Halt    (1);
    End;
  End;
