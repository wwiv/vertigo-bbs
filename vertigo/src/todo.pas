...
- add vertigo's downlinks to areas.bbs export?
...
- externalize qwk and file list compiler class.  qwk for vertigo/mis filelist
  for vertigo/mutil.  add compiler templates, file include, and new vs all
  generation for all.
...
- msg readers uses msgbase_ansi like vertigo 2
...
- Need ALL vertigo servers to hvae the option to auto-ban an IP address if it
...
- NodeSpy split chat
- GBBSCGI (or PHP DLL) [Grymmjack might have the only MBBSCGI copy]
...
- Vertigo-DOS rewrite or just code a file manager which would probably be a
...
- Template system similar to Vertigo 2 (ansiedit.ans ansiedit.ans.cfg)
...
7. Case result:
     ...
     Ask later:
        Do nothing.  Keep files so Vertigo asks on next login.
        Or also Vertigo could ask any time a MP menu command happens
...
- Msg editor can "post process" ANSIs to be 79 columns max in stored line
  length (bbs friendly)
- Rewrite of GBBSWEB or integrated HTML server?  still need a good designer
  that actually will put a lot of time into it
- Rewrite of ANSI template system (.ini files or vertigo2 format?)

=================================================

CODE RESTRUCTURE naming (possibly remove vertigo_ prefix):

vertigo
vertigo_records
vertigo_common (called db now)
vertigo_server_binkp
vertigo_server_telnet
vertigo_server_ftp
vertigo_server_pop3
vertigo_server
vertigo_client
vertigo_client_smtp
vertigo_client_ftp
vertigo_server_events
vertigo_cmd_server
vertigo_cmd_fidopoll
vertigo_cmd_terminal
vertigo_class_binkp
vertigo_class_qwk
vertigo_class_menudata
vertigo_class_msgbase
vertigo_class_msgbase_jam
vertigo_class_msgbase_squish
vertigo_class_arcview
vertigo_class_arcview_zip
vertigo_class_arcview_rar
vertigo_class_nodelist
vertigo_class_logging (on-the-fly log rolling, trimming, etc)
vertigo_config_filearea
vertigo_ansi_intro
vertigo_ansi_monitor
vertigo_ansi_terminal
vertigo_bbs_core
vertigo_bbs_msgbase
vertigo_bbs_filebase
vertigo_bbs_general
vertigo_bbs_doors
vertigo_bbs_user
vertigo_bbs_nodechat
vertigo_bbs_userchat
vertigo_bbs_sysopchat
vertigo_bbs_ansibox
vertigo_bbs_ansimenu
vertigo_bbs_ansihelp
vertigo_bbs_ansiinput
vertigo_bbs_ansidir
vertigo_bbs_lineedit
vertigo_bbs_fulledit
vertigo_bbs_menus
vertigo_bbs_io
vertigo_bbs_mplexecute
vertigo_bbs_mpltypes
vertigo_bbs_mplcommon
vertigo_bbs_mplcompile
