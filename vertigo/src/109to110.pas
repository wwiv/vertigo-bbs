...
Type
  ...
  OldConfigRec = Record               { VERTIGO.DAT in root BBS directory   }
    Version      : String[8];
...
Begin
  TextAttr := 15;
  ClrScr;
  WriteLn ('VERTIGO BBS VERSION 1.10 UPGRADE UTILITY');
  TextAttr := 8;
  WriteLn ('---------------------------------------');
  WriteLn;
  TextAttr := 7;
  WriteLn ('You must be using a current installation of Vertigo BBS 1.09 in');
...
Begin
  Assign (OldConfigFile, 'vertigo.dat');
  {$I-} Reset (OldConfigFile); {$I+}
  If IoResult <> 0 Then Begin
    WriteLn ('ERROR: Run this program from the root Vertigo BBS directory.');
    Halt(1);
  End;
...
Begin
  ...
  //COMMENT this out if vertigo.dat is being converted:
//  Assign (ConfigFile, 'vertigo.dat');
//  Reset  (ConfigFile);
