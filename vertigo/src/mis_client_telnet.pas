...
Begin
  ...
  Num := ND.GetFreeNode;
  Cmd := 'vertigo.exe -n' + strI2S(Num) + ' -TID' + strI2S(PassHandle) + ' -IP' + Client.FPeerIP + ' -HOST' + Client.FPeerName + #0;
...
Begin
  ...
  If PID = 0 Then Begin
    ...
    fpExecLP ('./vertigo', ['-n' + strI2S(Num), '-TID' + strI2S(Client.FSocketHandle), '-IP' + Client.FPeerIP, '-HOST' + Client.FPeerName]);
...
Begin
  ...
  Proc.CommandLine := 'vertigo -n' + strI2S(Num) + ' -IP' + Client.FPeerIP + ' -HOST' + Client.FPeerName;
