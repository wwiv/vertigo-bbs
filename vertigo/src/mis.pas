...
Begin
  Case bbsCfgStatus of
    cfgNotFound : If Not DaemonMode Then Begin
                    Console.WriteLine (#13#10 + 'ERROR: Unable to read VERTIGO.DAT.  This file must exist in the same');
                    Console.WriteLine ('directory as MIS or VERTIGOBBS environment location');
...
Var
  ...
  SID : TPID;
Begin
  WriteLn('- [MIS] Executing Vertigo Internet Server in daemon mode');
...
Const
  WinTitle = 'Vertigo Internet Server';
...
//      	#32 : LocalLogin;
    ...
    If (FocusPtr <> NIL) Then
      If FocusPtr.StatusUpdated Then Begin
        ...
        Count := 1;
      End Else
      If Count = 10 Then Begin  // force update every 10 seconds since vertigo
  ...
  Console.WriteLine ('Vertigo Internet Server Version ' + mysVersion);
  Console.WriteLine ('');
