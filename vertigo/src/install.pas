// ====================================================================
// Vertigo BBS Software               Copyright 1997-2012 By James Coyle
// ====================================================================
//
...
Begin
	...
	Repeat
		...
		Screen.WriteXY (1, A, 8, ' -' + strRep(' ', 75) + '--ú');
		Screen.WriteXY (1, B, 8, ' -' + strRep(' ', 75) + '--ú');
	...
	Repeat
		Dec (A, 2);
		Screen.WriteXY (1, 13, 8, strPadC(' -' + strRep(' ', A) + '-- ', 80, ' '));
		WaitMS(7);
	Until A = 0;
...
Begin
  ...
  Screen.WriteXY (1, 25, 8, strRep('-', 79));
	...
	Repeat
      ...
      BarPos := 0;
          ...
          5 : Screen.WriteXY (70, 25, 25, ' ');
          6 : Screen.WriteXY (71, 25, 27, ' ');
          7 : Screen.WriteXY (72, 25, 27, ' ');
          8 : Screen.WriteXY (73, 25, 27, ' ');
          9 : Screen.WriteXY (74, 25, 11, ' ');
          10: Screen.WriteXY (75, 25, 15, ' ');
				End;
			End;
      ...
      If Copy(Txt[Line + A]^, 4, 6) = '<ALPHA' Then
        LineColor := 9
      Else Begin
        If Copy(Txt[Line + A]^, 1, 21) = '| Vertigo BBS RELEASED' Then
          LineColor := 9
        Else
          LineColor := 7;
      End;
...
Begin
  Screen.WriteXYPipe (23, 13, 7, 45, '|08[|15X|08] |07Creating directories|08...');
...
Var
	...
	TLang 		: RecTheme;
Begin
  Screen.WriteXYPipe (23, 19, 7, 45, '|08[|15X|08] |07Updating data files|08...');

	Assign (CfgFile, Config.SystemPath + 'vertigo.dat');
	Reset  (CfgFile);
	Read	 (CfgFile, Cfg);
...
Begin
  ...
  ExtractFile (14, '|08[|15X|08] |07Installing root files|08...',    'install_data', 'ROOT',   Config.SystemPath);
	ExtractFile (15, '|08[|15X|08] |07Installing display files|08...', 'install_data', 'TEXT',   Lang.TextPath);
	ExtractFile (16, '|08[|15X|08] |07Installing menu files|08...',    'install_data', 'MENUS',  Lang.MenuPath);
	ExtractFile (17, '|08[|15X|08] |07Installing script files|08...',  'install_data', 'SCRIPT', Config.ScriptPath);
	ExtractFile (18, '|08[|15X|08] |07Installing data files|08...',    'install_data', 'DATA',   Config.DataPath);
  ExtractFile (19, '|08[|15X|08] |07Installing documentation|08...', 'install_data', 'DOCS',   Config.SystemPath + 'docs' + PathChar);
  ...
  Clear_Screen;
  Screen.WriteLine ('Switch to the Vertigo directory (' + Config.SystemPath + ') and then:');
  Screen.WriteLine('');
  {$IFDEF WINDOWS}
    Screen.WriteLine ('Type "VERTIGO" to run Vertigo in local mode (do this first to create SysOp account)');
    Screen.WriteLine ('Type "VERTIGO -CFG" to run the configuration utility');
  {$ENDIF}
  {$IFDEF LINUX}
    Screen.WriteLine ('Please read linux.install.txt for installation instructions');
    Screen.WriteLine ('and notes on using Vertigo under Linux');
    Screen.WriteLine('');
    Screen.WriteLine ('Set your terminal to 80x25 lines with an IBM characterset font!');
    Screen.WriteLine('');
    Screen.WriteLine ('Type "./vertigo" to login (do this first to make your SysOp account)');
    Screen.WriteLine ('Type "./vertigo -cfg" to run the configuration utility');
  {$ENDIF}
  {$IFDEF DARWIN}
    Screen.WriteLine ('Please read osx.install.txt for installation instructions');
    Screen.WriteLine ('and notes on using Vertigo under OSX');
    Screen.WriteLine('');
    Screen.WriteLine ('Set your terminal to 80x25 lines with an IBM characterset font!');
...
Begin
  ...
  Screen.WriteXYPipe (19, 23, 7, 64, 'Press |08[|15F2|08] |07to begin install or |08[|15ESC|08] |07to Quit');
	...
	{$IFDEF UNIX}
    Config.SystemPath := '/opt/vertigo/';
  {$ELSE}
    Config.SystemPath := 'c:\vertigo\';
  {$ENDIF}
